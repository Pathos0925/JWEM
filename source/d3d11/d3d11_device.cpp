...
HRESULT STDMETHODCALLTYPE D3D11Device::CreateTexture2D(const D3D11_TEXTURE2D_DESC *pDesc, const D3D11_SUBRESOURCE_DATA *pInitialData, ID3D11Texture2D **ppTexture2D)
{
	...
	JWEmSingleton->originalDevice = _orig;

	if (pDesc == nullptr)
	{
		LOG(DEBUG) << "pDesc is nullprt";
		auto toReturn = _orig->CreateTexture2D(pDesc, pInitialData, ppTexture2D);
		return toReturn;
	}
	if (JWEmSingleton->MonitorTextures)
	{
		LOG(DEBUG) << "CreateTexture2D";
		LOG(DEBUG) << "H: " << pDesc->Height << " W: " << pDesc->Width;
		LOG(DEBUG) << "Format: " << pDesc->Format;
	}
	const int texSize = pDesc->Width * pDesc->Height;

	uint32_t partialCrc = 0;


	if (!JWEmSingleton->Enabled)
	{
		auto toReturn = _orig->CreateTexture2D(pDesc, pInitialData, ppTexture2D);
		return toReturn;
	}
	...
	uint32_t initialDataCrc = 0;
	//texture, normalmap, packed formats


	//TODO
	//DXGI_FORMAT_BC7_UNORM_SRGB
	if (pInitialData != nullptr
		&& pInitialData[0].pSysMem != nullptr
		&& pDesc != nullptr
		&& pDesc->Height >= JWEmSingleton->RecordTextureCutoffSize
		&& pDesc->Width >= JWEmSingleton->RecordTextureCutoffSize
		//&& pDesc->Height == 1024
		//&& pDesc->Width == 1024
		&& (pDesc->Format == DXGI_FORMAT::DXGI_FORMAT_BC7_UNORM_SRGB
			|| pDesc->Format == DXGI_FORMAT::DXGI_FORMAT_BC5_UNORM
			|| pDesc->Format == DXGI_FORMAT::DXGI_FORMAT_BC7_UNORM
			|| pDesc->Format == DXGI_FORMAT::DXGI_FORMAT_BC1_TYPELESS)

		)
	{

		if (JWEmSingleton->multiThreadCrc)
		{
			LOG(DEBUG) << "Queueing CRC for: " << *ppTexture2D;
			unsigned char * copyInto = new unsigned char[texSize];
			memcpy(copyInto, pInitialData[0].pSysMem, texSize);
			JWEmSingleton->Start_CRCTextureData(copyInto, texSize, *ppTexture2D);
			//JWEmSingleton->crcQueue.insert(std::pair<unsigned char*, int>(copyInto, texSize));
		...
		{
			//auto partialCrc = JWEmSingleton->PartialCrcInitialData(pInitialData[0].pSysMem, texSize);

			initialDataCrc = JWEmSingleton->CrcInitialData(pInitialData[0].pSysMem, texSize);

			//after creation, pDesc and pInitialData are null
			auto result = _orig->CreateTexture2D(pDesc, pInitialData, ppTexture2D);

			if (pDesc->Height >= JWEmSingleton->RecordTextureCutoffSize && pDesc->Width >= JWEmSingleton->RecordTextureCutoffSize)
			{
				JWEmSingleton->addTex(*ppTexture2D, initialDataCrc);
			}
			return result;
		}
	}
